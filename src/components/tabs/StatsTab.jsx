  ...
  return (
    <div className="ring-wrap">
      <svg width="120" height="120" viewBox="0 0 120 120">
        ...
        />
        <text x="60" y="67" textAnchor="middle" fontSize="24" fontWeight="800" fill="var(--ink)">
          {pct}%
        </text>
      </svg>
...
export default function StatsTab({ state, weekKey }) {
  ...
  // Прогресс по привычкам (доля дней в неделю).
      ...
      <div className="card">
        ...
        <div className="stat-cards">
          <div className="stat-card">
            ...
            <div className="lbl">задачи выполнены</div>
          </div>
          <div className="stat-card">
            <div className="big">
              {habits.done}/{habits.total}
            </div>
            <div className="lbl">отметки привычек</div>
          </div>
          <div className="stat-card">
            <div className="big">{Math.round(steps.avg).toLocaleString('ru-RU')}</div>
            <div className="lbl">шагов в среднем</div>
          </div>
          <div className="stat-card">
            <div className="big">{steps.daysHit}</div>
            <div className="lbl">дней с целью по шагам</div>
          </div>
        </div>
      </div>
        ...
        <div className="card">
          ...
          <ResponsiveContainer width="100%" height={200}>
            <BarChart data={sleepData} margin={{ top: 10, right: 10, left: -20, bottom: 0 }}>
              <CartesianGrid strokeDasharray="3 3" stroke="var(--c2)" />
              <XAxis dataKey="day" stroke="var(--ink)" />
              <YAxis domain={[0, 12]} stroke="var(--ink)" />
              <Tooltip />
